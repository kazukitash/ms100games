...
class GameScene extends Scene
  constructor: ->
    ...
    @number.push(Math.floor(Math.random() * 5999 + 4000))
    @number.push(Math.floor(Math.random() * 329 + 70)) for num in [1..9]
...
class GameOverScene extends Scene
  constructor: (ans) ->
    ...
    @number = []
    @number.push @ans
    (@number.push(@ans + Math.floor(Math.random() * 999 - 400))) for num in [0..2]
    @number.shuffle
    console.log @number
