...
class GameScene extends Scene
  constructor: ->
    ...
    @number.push(Math.floor(Math.random() * 8999999 + 1000000))
    @number.push(Math.floor(Math.random() * 9 + 1)) for num in [1..9]
  ...
  onenterframe: ->
    if @age % 30 is 0 and @n < 10
      @label.text = @number[@n]
      @n++
    else if @age % 30 is 0
      core.gameOverScene = new GameOverScene(@ans)
...
class GameOverScene extends Scene
  constructor: (ans) ->
    ...
    @number = []
    @number.push @ans.toFixed(2)
    @number.push((@ans + Math.random() * 20 - 10).toFixed(2)) for num in [0..2]
    @number.shuffle
    console.log @number
