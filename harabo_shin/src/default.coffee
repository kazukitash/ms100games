...
class Traveler extends Sprite
  constructor: (iraira)->
    super(iraira[0],iraira[1])
    @image = core.assets[iraira[2]]
    @frame = 0
    @moveTo Math.random() * (HQ_GAME_WIDTH - @width), Math.random() * (HQ_GAME_HEIGHT - @height)
...
class Travelers extends Group
  ...
  onenterframe: ->
    ...
    if core.frame % 90 == 0
      traveler = new Traveler(Iraira[Math.floor(Math.random() * 2)])
      @addChild traveler
