...
(function() {
  var Angle, Ball, GameOverScene, GameScene, Player, Power, TitleScene, core,
    ...
    GameScene.prototype.onenterframe = function() {
      ...
      if (this.touchCount.length > 1) {
        if (this.touchCount[1] + 2 * core.fps === core.frame) {
          power = this.power.percentage() * 20 + 4;
        ...
        if (this.ball.vx === 0 && this.ball.vy === 0 && this.touchCount[1] + 4 * core.fps < core.frame) {
          core.gameOverScene = new GameOverScene(this.ball.x);
  ...
  GameOverScene = (function(_super) {
    ...
    function GameOverScene(score) {
      ...
      this.addChild(this.bg);
      if (score > 1025 && score < 1077) {
        this.label = new Label("素晴らしい");
      } else if (score > 981 && score < 1122) {
        this.label = new Label("よくやった");
      } else if (score > 944 && score < 1161) {
        this.label = new Label("いいね");
      } else if (score > 912 && score < 1191) {
        this.label = new Label("まあまあだね");
      } else if (score > 872 && score < 1229) {
        this.label = new Label("もう少し");
      } else if (score > 832 && score < 1269) {
        this.label = new Label("まだまだだね");
      } else {
        this.label = new Label("ダメ");
      }
